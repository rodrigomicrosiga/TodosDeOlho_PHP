...
echo "Importando arquivo [05_PropostasPrograma.csv]\n";
...
if (($handle = fopen("05_PropostasPrograma.csv", "r")) !== FALSE) {
	...
	$stmt = mysqli_prepare($conn,"INSERT INTO PROPOSTAS ( ANO_PROPOSTA,NR_PROPOSTA,ANO_CONVENIO,".
	"NR_CONVENIO,TX_MODALIDADE,TX_SITUACAO,TX_SUBSITUACAO,CD_PROGRAMA,CD_ACAO_PROGRAMA,NM_PROGRAMA,".
	"CD_ORGAO_SUPERIOR,NM_ORGAO_SUPERIOR,CD_ORGAO_CONCEDENTE,NM_ORGAO_CONCEDENTE,CD_IDENTIF_PROPONENTE,".
	...
	"ID_PROPOSTA,ID_CONVENIO,ID_PROP_PROGRAMA) ".
	"VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ");
	...
	while (($data = fgetcsv($handle, 8192, ';' , '"' , '"' )) !== FALSE) {
		...
		$refarg = array($stmt, 'iiiissssssisisssssssssssiisiiiiiisssssssssssiii');
		for ($c=0; $c < $num; $c++)
            $refarg[] =& $data[$c];
